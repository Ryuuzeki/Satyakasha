...
const getNetgroup = (address, secret) => {
    ...
    return lisk_cryptography_1.utils.hash(netgroupBytes).readUInt32BE(0);
};
...
const getBucketId = (options) => {
    ...
    if (network !== NETWORK.NET_IPV4) {
        return lisk_cryptography_1.utils.hash(Buffer.concat([secretBytes, networkBytes])).readUInt32BE(0) % bucketCount;
    }
    ...
    const k = peerType === PEER_TYPE.NEW_PEER && sourceBytes
        ...
        : lisk_cryptography_1.utils.hash(Buffer.concat([secretBytes, networkBytes, addressBytes])).readUInt32BE(0) %
            firstMod;
    ...
    return lisk_cryptography_1.utils.hash(bucketBytes).readUInt32BE(0) % bucketCount;
};
